...
rice-adapted model variants. Training epochs are scaled by proteome size
(44 and 16 epochs, respectively), with both models trained for
approximately 10,400 steps to ensure comparable optimisation.
...
(16 epochs) and *Chlamydomonas*-adapted (44 epochs) variants, plotted across
...
acids. Because the two proteomes differ substantially in size, we scaled
training epochs inversely with proteome size—44 epochs for *C. reinhardtii*
and 16 for *O. sativa*—so that both models were trained for approximately
10,400 steps, ensuring comparable optimisation.
...
得到衣藻适应性模型和水稻适应性模型。训练 epoch 数按蛋白质组大小反比调整
（分别为 44 和 16 epoch），两者均训练约 10,400 步以保证优化程度可比。
...
水稻适应性（16 epoch）和衣藻适应性（44 epoch）ESM2 变体在 polyQ 长度
...
15% 的残基并要求模型恢复原始氨基酸。由于两个蛋白质组大小差异显著，按蛋白质组
大小反比调整训练 epoch 数（衣藻 44 epoch，水稻 16 epoch），使两者均训练约
10,400 步，保证优化程度可比。
